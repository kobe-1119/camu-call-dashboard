...
def classify(row):
    ...
    # P1c: NEW — Spanish appointment confirmations
    # Agent discussing appointment details (cita, appointment, confirm) + patient says sí/ok/gracias
    ...
    if has_phrase(at, agent_appt_phrases) and has_phrase(ut, spanish_confirms) and user_words <= 15:
        # Make sure they're not saying "no" predominantly
    ...
    # P1d: NEW — Third party confirming appointment
    # "his father", "her daughter" etc + confirming attendance
    ...
    # P5b: NEW — Third party with callback scheduling
    ...
    # NEW — repeated "aló" with no real conversation
    ...
    # NEW — mixed language with mostly inaudible
    ...
    if has_phrase(ut, cb_explicit):
        # NEW — check agent also confirms callback
        ...
        # Still count it if patient clearly says callback words
    ...
    # NEW — Agent confirms callback in transcript even if patient words are sparse
    ...
    # P8b: NEW — Patient wants to speak to front desk / human
    ...
    # P10b: NEW — short but unclear responses (11-15 words, not fitting above)
    if user_words > 10:
        return "Other - Engaged/Inconclusive", "Patient engaged but no clear outcome"
...
# Test on March 13 data
CSV_PATH = "/sessions/affectionate-intelligent-ride/mnt/Confido Call Reviews/Raw Data (Updated Daily)/outgoing_calls_2026-03-13_10-12.csv"
df = pd.read_csv(CSV_PATH, dtype=str)
df.columns = df.columns.str.strip()

results = []
for idx, row in df.iterrows():
    outcome, sub = classify(row)
    results.append({"outcome": outcome, "sub": sub})

rdf = pd.DataFrame(results)
print("=== March 13 — V2 Classifier ===")
print(f"Total: {len(rdf)}")
print(f"\nUnclassified: {len(rdf[rdf['sub']=='Unclassified'])}")
print(f"\nOutcome Distribution:")
print(rdf["outcome"].value_counts().to_string())
print(f"\nSub-Category Distribution:")
print(rdf["sub"].value_counts().to_string())
